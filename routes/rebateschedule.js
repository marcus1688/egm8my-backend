  ...
  async (req, res) => {
    try {
      ...
      res.status(200).json({
        success: true,
        message: {
          en: "Rescue schedule updated successfully",
          zh: "复活分计划更新成功",
        },
        data: schedule,
      });
    } catch (error) {
      console.error("Error updating rescue schedule:", error);
      res.status(500).json({
        success: false,
        message: {
...
async function runRebateCalculation() {
  try {
    ...
    console.error("Rescue calculation error:", error);
    throw error;
  }
}
      ...
      if (stats.totalwinlose > 0) {
        ...
        if (stats.totalRebate >= 1) {
          ...
          if (!shouldClaim) {
            ...
            console.log(
              `Rescue processed immediately for ${username}: ${stats.totalRebate}`
            );
          } else {
            console.log(
              `Rescue pending for ${username}: ${stats.totalRebate} (wallet: ${user.wallet})`
            );
          }
        }
      }
    }
    console.log("Rescue calculation completed");
  } catch (error) {
    console.error("Win/Lose rescue calculation error:", error);
    throw error;
  }
}
