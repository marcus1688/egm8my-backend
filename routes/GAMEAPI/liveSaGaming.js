...
router.post("/api/sagaming/query", async (req, res) => {
  try {
    const TokenTime = moment.utc().format("YYYYMMDDHHmmss");

    const game = "baccarat";

    const TokenQueryString = `method=QueryBetLimit&Key=${saGamingSecurity}&Time=${TokenTime}&Currency=MYR&Gametype=${game}`;
    ...
    } else if (result.QueryBetLimitResponse) {
      TokenerrorMsgId = result.QueryBetLimitResponse.ErrorMsgId[0];
    }
    ...
    return res.status(200).json({
      success: false,
      message: {
        en: "SA GAMING: Game launch failed. Please try again or customer service for assistance.",
        zh: "SA GAMING: 游戏启动失败，请重试或联系客服以获得帮助。",
        ms: "SA GAMING: Pelancaran permainan gagal. Sila cuba lagi atau hubungi khidmat pelanggan untuk bantuan.",
      },
    });
...
router.post("/api/sagaming/setbetlimit", async (req, res) => {
  try {
    const TokenTime = moment.utc().format("YYYYMMDDHHmmss");

    const game = "10104581000120004, 10304581000120004, 10504581000010004";

    const TokenQueryString = `method=SetAllBetLimit&Key=${saGamingSecurity}&Time=${TokenTime}&Username=jk0ytzdi&Currency=MYR&ListSet=${game}`;
...
router.post("/api/sagaming/launchGame", authenticateToken, async (req, res) => {
  try {
    ...
    const TokenQueryString = `method=LoginRequest&Key=${saGamingSecurity}&Time=${TokenTime}&Username=${user.userServerId}&CurrencyType=MYR`;
    ...
    const LaunchGameQueryString = `username=${user.userServerId}&token=${token}&lobby=A12795&lang=${lang}&returnurl=${webURL}`;
