...
router.post("/api/yggdrasil/updategameid", async (req, res) => {
  try {
    ...
    missingByIdGames.forEach((apiGame) => {
      ...
      if (matchingDbGame) {
        ...
        updatePromises.push(
          GameYGGDrasilGameModal.updateOne(
    ...
    console.error("YGGDRASIL error comparing game names:", error.message);
    return res.status(200).json({
      success: false,
      message: {
        en: "YGGDRASIL: Game comparison failed. Please try again or contact customer service for assistance.",
        zh: "YGGDRASIL: 游戏比较失败，请重试或联系客服以获得帮助。",
        ms: "YGGDRASIL: Perbandingan permainan gagal. Sila cuba lagi atau hubungi khidmat pelanggan untuk bantuan.",
        zh_hk: "YGGDRASIL: 遊戲比較失敗，請重試或聯絡客服以獲得幫助。",
        id: "YGGDRASIL: Perbandingan permainan gagal. Silakan coba lagi atau hubungi layanan pelanggan untuk bantuan.",
      },
    });
  }
});
...
router.post("/api/yggdrasil/comparegamenames", async (req, res) => {
  try {
    ...
    return res.status(200).json({
      success: false,
      message: {
        en: "YGGDRASIL: Game name comparison failed. Please try again or contact customer service for assistance.",
        zh: "YGGDRASIL: 游戏名称比较失败，请重试或联系客服以获得帮助。",
        ms: "YGGDRASIL: Perbandingan nama permainan gagal. Sila cuba lagi atau hubungi khidmat pelanggan untuk bantuan.",
        zh_hk: "YGGDRASIL: 遊戲名稱比較失敗，請重試或聯絡客服以獲得幫助。",
        id: "YGGDRASIL: Perbandingan nama permainan gagal. Silakan coba lagi atau hubungi layanan pelanggan untuk bantuan.",
      },
    });
  }
});

router.post("/api/yggdrasil/comparegame", async (req, res) => {
  try {
    const sign = generateSignature({
      brandId: dctyggdrasilGameBrandID,
      apiKey: dctyggdrasilGameKey,
    ...
    console.error("YGGDRASIL error launching game:", error.message);
    return res.status(500).json({
      success: false,
      message: {
        en: "YGGDRASIL: Game launch failed. Please try again or customer service for assistance.",
        zh: "YGGDRASIL: 游戏启动失败，请重试或联系客服以获得帮助。",
        ms: "YGGDRASIL: Pelancaran permainan gagal. Sila cuba lagi atau hubungi khidmat pelanggan untuk bantuan.",
        zh_hk: "YGGDRASIL: 遊戲啟動失敗，請重試或聯絡客服以獲得幫助。",
        id: "YGGDRASIL: Peluncuran permainan gagal. Silakan coba lagi atau hubungi layanan pelanggan untuk bantuan.",
      },
    });
  }
});

router.post("/api/yggdrasil/getprovidergamelist", async (req, res) => {
  try {
    const sign = generateSignature({
      brandId: dctyggdrasilGameBrandID,
      apiKey: dctyggdrasilGameKey,
...
router.post("/api/yggdrasil/getgamelist", async (req, res) => {
  try {
    const games = await GameYGGDrasilGameModal.find({
      $and: [
        {
    ...
    console.error("YGGDRASIL Error fetching game list:", error.message);
    return res.status(200).json({
      success: false,
      message: {
        en: "YGGDRASIL: Unable to retrieve game lists. Please contact customer service for assistance.",
        zh: "YGGDRASIL: 无法获取游戏列表，请联系客服以获取帮助。",
        ms: "YGGDRASIL: Tidak dapat mendapatkan senarai permainan. Sila hubungi khidmat pelanggan untuk bantuan.",
        zh_hk: "YGGDRASIL: 無法獲取遊戲列表，請聯絡客服以獲取幫助。",
        id: "YGGDRASIL: Tidak dapat mengambil daftar permainan. Silakan hubungi layanan pelanggan untuk bantuan.",
      },
    });
  }
});
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (user.gameLock.yggdrasil.lock) {
        return res.status(200).json({
          success: false,
          message: {
      ...
      const response = await axios.post(
        `${dctyggdrasilGameAPIURL}/dcs/loginGame`,
        payload,
        {
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.data.code !== 1000) {
        console.log("YGGDRASIL fail to launch game with error", response.data);
...
router.post("/api/yggdrasil/getturnoverforrebate", async (req, res) => {
  try {
    const { date } = req.body;
    ...
    const records = await SlotDCTGameYggDrasilModal.find({
      createdAt: {
        $gte: startDate,
        $lt: endDate,
      },
    ...
    records.forEach((record) => {
      ...
      if (!actualUsername) {
        console.warn(`YGGDRASIL User not found for gameId: ${gameId}`);
        return;
      }
    ...
    console.log("YGGDRASIL: Failed to fetch win/loss report:", error.message);
    return res.status(500).json({
      success: false,
      message: {
        en: "YGGDRASIL: Failed to fetch win/loss report",
        zh: "YGGDRASIL: 获取盈亏报告失败",
      },
    });
  }
});
...
router.get(
  "/admin/api/yggdrasil/:userId/dailygamedata",
...
router.get(
  "/admin/api/yggdrasil/:userId/gamedata",
  authenticateAdminToken,
  async (req, res) => {
    try {
      const { startDate, endDate } = req.query;
      ...
      records.forEach((record) => {
        ...
        if (
          ...
          if (gameCat["YGGDRASIL"]) {
            totalTurnover += gameCat["YGGDRASIL"].turnover || 0;
            totalWinLoss += gameCat["YGGDRASIL"].winloss || 0;
          }
        }
      });
...
router.get(
  "/admin/api/yggdrasil/dailykioskreport",
  authenticateAdminToken,
  async (req, res) => {
    try {
      const { startDate, endDate } = req.query;
      ...
      console.error("YGGDRASIL: Failed to fetch win/loss report:", error);
      return res.status(500).json({
        success: false,
        message: {
          en: "YGGDRASIL: Failed to fetch win/loss report",
...
router.get(
  "/admin/api/yggdrasil/kioskreport",
  authenticateAdminToken,
  async (req, res) => {
    try {
      const { startDate, endDate } = req.query;
      ...
      records.forEach((record) => {
        ...
        if (
          ...
          if (gameCat["YGGDRASIL"]) {
            totalTurnover += Number(gameCat["YGGDRASIL"].turnover || 0);
            totalWinLoss += Number(gameCat["YGGDRASIL"].winloss || 0) * -1;
          }
        }
      });
