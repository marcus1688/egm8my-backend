...
// async function updateHabaneroApiRouteTimestamps() {
//   try {
...
//     const gameIds = apiGames.map((game) => game.KeyName);
...
//       const result = await GameHabaneroGameModal.collection.updateOne(
//         { gameID: gameId },
//         {
//           $set: {
//             createdAt: timestamp,
...
//           (game) => game.KeyName === gameId
//         );
//         console.log(
//           `Updated Habanero gameID ${gameId} (${
//             correspondingApiGame?.Name
...
//     console.log("Habanero API route timestamp update completed!");

//     // Verify the updates by fetching and displaying the results
//     const updatedGames = await GameHabaneroGameModal.find(
//       { gameID: { $in: gameIds } },
...
//       const correspondingApiGame = apiGames.find(
//         (apiGame) => apiGame.KeyName === game.gameID
//       );
//       console.log(
//         `${index + 1}. GameID: ${
//           game.gameID
...
//           game.hot
//         }, API Name: ${correspondingApiGame?.Name || "N/A"}, DB Name: ${
//           game.gameNameEN
//         }`
//       );
//     });
...
//     );
//   } catch (error) {
//     console.error("Error updating Habanero API route timestamps:", error);
//   }
// }

// // Call the function
// updateHabaneroApiRouteTimestamps();
...
router.post("/api/habanero/comparegame", async (req, res) => {
  try {
    ...
    // Return missing games with KeyName and game details
    const missingGamesInfo = missingGames.map((game) => ({
      ...
      translatedNames: game.TranslatedNames,
    }));
  ...
  try {
    ...
    return res.status(200).json({
      ...
      message: {
        ...
        zh_hk: "HABANERO: 無法獲取遊戲列表，請聯絡客服以獲取幫助。",
...
router.post("/api/habanero/auth", async (req, res) => {
  try {
    ...
    const currentUser = await User.findOne(
      { gameId: username, habaneroGameToken: token },
  ...
  try {
    ...
    if (fundtransferrequest.isrecredit === true) {
      ...
      const [updatedUser] = await Promise.all([
        User.findOneAndUpdate(
          { gameId: username },
          { $inc: { wallet: roundToTwoDecimals(parseFloat(creditAmount)) } },
          { new: true, projection: { wallet: 1 } }
        ),
        SlotHabaneroModal.findOneAndUpdate(
          {
          ...
          },
          {
            $set: {
              ...
              roundId: gameinstanceid,
              settleamount: roundToTwoDecimals(parseFloat(creditAmount)),
              bet: true,
              settle: true,
              recredit: true,
            },
    ...
    if (fundinfo[0].isbonus === true) {
      ...
      if (bonusAmount < 0) {
        await SlotHabaneroModal.findOneAndUpdate(
          ...
          },
          {
            $setOnInsert: {
              ...
              uniqueId: transactionInfo.transferid,
              roundId: gameinstanceid,
              settleamount: roundToTwoDecimals(
                Math.abs(parseFloat(bonusAmount))
              ),
              settle: true,
              bet: true,
            },
        ...
        const [updatedUser] = await Promise.all([
          User.findOneAndUpdate(
            { gameId: username },
            { $inc: { wallet: roundToTwoDecimals(parseFloat(amountToAdd)) } },
            { new: true, projection: { wallet: 1 } }
          ),
          SlotHabaneroModal.create({
            ...
            betamount: 0,
            bet: true,
            settleamount: roundToTwoDecimals(parseFloat(amountToAdd)),
            settle: true,
          }),
        ]);
      ...
      if (fundtransferrequest.isretry) {
        ...
        if (existingDebit && !existingCredit) {
          const [updatedUserBalance] = await Promise.all([
            User.findOneAndUpdate(
              { gameId: username },
              { $inc: { wallet: roundToTwoDecimals(creditInfo.amount) } },
              { new: true, projection: { wallet: 1 } }
            ),
            SlotHabaneroModal.findOneAndUpdate(
              {
      ...
      try {
        const [updatedUser, transaction] = await Promise.all([
          User.findOneAndUpdate(
            {
              gameId: username,
              wallet: { $gte: Math.abs(debitInfo.amount) },
            },
            { $inc: { wallet: roundToTwoDecimals(resultBalance) } },
            { new: true, projection: { wallet: 1 } }
          ),
          SlotHabaneroModal.create({
            username: username,
            uniqueId: debitInfo.transferid,
            roundId: gameinstanceid,
            betamount: Math.abs(debitInfo.amount),
            bet: true,
            settleamount: roundToTwoDecimals(creditInfo.amount),
            settle: true,
      ...
      try {
        const [updatedUser, firstTransaction] = await Promise.all([
          User.findOneAndUpdate(
            {
              gameId: username,
              wallet: { $gte: Math.abs(roundToTwoDecimals(transactionAmount)) },
            },
        ...
        if (firstTransaction) {
          await SlotHabaneroModal.create({
            ...
            roundId: gameinstanceid,
            uniqueId: transactionInfo.transferid,
            betamount: Math.abs(roundToTwoDecimals(transactionAmount)),
            bet: true,
            settleamount: 0,
            settle: true,
          });
