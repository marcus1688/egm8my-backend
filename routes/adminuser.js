    ...
    try {
      ...
      const adminStats = await Promise.all(
        admins.map(async (admin) => ({
          ...
          handlingTime: await calculateAvgHandlingTime(
            admin.username,
            dateFilter
          ),
          stats: {
            deposit: {
              approvedQty: depositMap.get(admin.username)?.approvedQty || 0,
              rejectedQty: depositMap.get(admin.username)?.rejectedQty || 0,
              revertedQty: depositMap.get(admin.username)?.revertedQty || 0,
              approvedAmount:
                depositMap.get(admin.username)?.approvedAmount || 0,
            },
            withdraw: {
              approvedQty: withdrawMap.get(admin.username)?.approvedQty || 0,
              rejectedQty: withdrawMap.get(admin.username)?.rejectedQty || 0,
              revertedQty: withdrawMap.get(admin.username)?.revertedQty || 0,
              approvedAmount:
                withdrawMap.get(admin.username)?.approvedAmount || 0,
            },
            bonus: {
              approvedQty: bonusMap.get(admin.username)?.approvedQty || 0,
              rejectedQty: bonusMap.get(admin.username)?.rejectedQty || 0,
              revertedQty: bonusMap.get(admin.username)?.revertedQty || 0,
              approvedAmount: bonusMap.get(admin.username)?.approvedAmount || 0,
            },
            cash: {
